"""A light-weight wrapper for the Sanskrit WordNet API"""
...
load_dotenv()
SANSKRITWORDNET_TOKEN = os.getenv("SANSKRITWORDNET_TOKEN", None)
...
class Lemmas:
    def __init__(
            self, session, host, lemma=None, transliteration=None, pos=None, morpho=None, uri=None, mw=None, token=None
    ):
        self.session = session
        self.host = host
        if not transliteration:
            ...
            self.lemma = None
        if not lemma:
            self.transliteration = f"{transliteration}/" if transliteration else "*/"
        else:
    ...
    def get(self):
        if self.json is None:
            if self.uri is not None:
                ...
                self.json = self.session.get(
                    f"{self.host}/api/mw/{self.mw}?format=json"
                ).json()["results"]
            else:
                if not self.lemma:
                    query = self.transliteration
                else:
                    query = self.lemma
                self.json = []
                results = self.session.get(
                    f"{self.host}/api/lemmas/{query}{self.pos}{self.morpho}?format=json"
                ).json()
                while results:
                    self.json.extend(results["results"])
                    if results["next"]:
...
class SanskritWordNet:
    ...
    @lru_cache(maxsize=None)
    def lemmas(self, lemma=None, transliteration=None, pos=None, morpho=None):
        return Lemmas(self.session, self.host, lemma=lemma, transliteration=transliteration, pos=pos, morpho=morpho)
    ...
    @lru_cache(maxsize=None)
    def lemmas_by_mw(self, mw):
        return Lemmas(self.session, self.host, mw=mw)
